...
const moduleIndex = computed(() =>
  modules.value.find((m) => m.slug === route.params.moduleSlug),
);
const itemIndex = computed(
  () =>
    moduleIndex.value?.items.findIndex(
      (item) => item.date === route.params.date,
    ) ?? -1,
);
const item = computed(() =>
...
const newer = computed(() =>
  itemIndex.value > 0
    ? moduleIndex.value?.items[itemIndex.value - 1]
    : undefined,
);
...
function selectDate(date: string) {
  if (moduleIndex.value && date)
    router.push(`/${moduleIndex.value.slug}/${date}`);
}
  ...
  <main id="main-content" class="detail">
    ...
    <template v-else-if="moduleIndex && item">
      ...
      <div class="detail__body">
        <MarkdownRenderer
          v-if="moduleIndex.renderer === 'markdown'"
